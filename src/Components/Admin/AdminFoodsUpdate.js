...
function AdminFoodsUpdate({ id }) {
    ...
    useEffect(() => {
        fetch(`http://localhost:3030/foods/${id}`, { method: "GET" })
            .then((res) => res.json())
            .then((res) => {
                setData(res);
            });
    ...
    const handleImageUpload = async (e) => {
      ...
      try {
          ...
          setData({...data, img: uploadedUrls[0]});
    ...
    const updateFoods = () => {
        ...
        fetch(`http://localhost:3030/foods/${id}`, {
            ...
            .then(() => {
                // Close the modal
                const modalElement = document.getElementById(`updateFoodModal-${id}`);
...
export default AdminFoodsUpdate;
